...
<ul class="list">
   <?php for ($i = 0; $i < count($list['items']); ++$i) {
      echo sprintf('
         <li class="list-item">
            <p class="title">%s</p>
            <input type="checkbox" class="cbDel" value="%2$s">
            <a class="btn" href="%3$s%2$s">update</a>
         </li>', $list['items'][$i]['snippet']['title'], $list['items'][$i]['id'], baseUrl('playlist/update/') );
   } ?>
</ul>
<ul class="pagination">
   <?php
   if (isset($list['prevPageToken']))
      echo sprintf('<li><a href="%s" class="btn">previous</a></li>', baseUrl('playlist/index/'.$list['prevPageToken']) );
   if (isset($list['nextPageToken']))
      echo sprintf('<li><a href="%s" class="btn">next</a></li>', baseUrl('playlist/index/'.$list['nextPageToken']) );
   ?>
</ul>
<div class="notif"></div>
<script>
   document.getElementById('exe').addEventListener('click', function() {
      ...
      if (aid == false) {
         message('Choose some playlists to delete');
         return;
      }
      else if (aid.length == 1) aid = aid[0];

      let xhr = new XMLHttpRequest();
      xhr.open('GET', '<?= baseUrl('playlist/delete/') ?>' + JSON.stringify(aid) );
      xhr.send();
